...
export default function OrganisationsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      for (const org of orgsList) {
        const roundSnap = await getDocs(collection(db, 'Organisations', org.id, 'Rounds'))
  ...
  const getCurrentRoundShort = (orgId: string) => {
    ...
    const active = rounds.find(r => r.orgId === orgId && r.startDate.toDate() <= today && r.endDate.toDate() >= today)
